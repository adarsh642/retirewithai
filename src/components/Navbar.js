...
const Navbar = () => {
  ...
  return (
    <aside
      className="sticky top-0 z-20 flex h-screen w-64 flex-col border-r border-solid border-surface bg-background/80 p-6 backdrop-blur-md ring-1 ring-white/5 shadow-[0_8px_24px_rgba(0,0,0,0.35)]"
      style={{ backgroundColor: 'rgba(18, 11, 28, 0.80)' }}
    >
  ...
  <nav className="flex flex-1 flex-col gap-1 text-sm font-medium text-on-surface-variant">
        <Link className={`${linkBase} ${isActive('/dashboard') ? 'bg-surface text-white border border-white/10' : ''}`} href="/dashboard">
          <span className="material-symbols-outlined">dashboard</span>
          Dashboard
        </Link>
        <Link className={`${linkBase} ${isActive('/goals') ? 'bg-surface text-white border border-white/10' : ''}`} href="/goals">
          <span className="material-symbols-outlined">flag</span>
          Goals
        </Link>
        <Link className={`${linkBase} ${isActive('/setupplan') ? 'bg-surface text-white border border-white/10' : ''}`} href="/setupplan">
          <span className="material-symbols-outlined">playlist_add_check</span>
          Setup Plan
        </Link>
       
        <Link className={`${linkBase} ${isActive('/projection') ? 'bg-surface text-white border border-white/10' : ''}`} href="/projection">
          <span className="material-symbols-outlined">trending_up</span>
          Projection
        </Link>
        <Link className={`${linkBase} ${isActive('/reports') ? 'bg-surface text-white border border-white/10' : ''}`} href="/reports">
          <span className="material-symbols-outlined">insights</span>
          Reports
        </Link>
        <Link className={`${linkBase} ${isActive('/taxoptimize') ? 'bg-surface text-white border border-white/10' : ''}`} href="/taxoptimize">
          <span className="material-symbols-outlined">percent</span>
          Tax Optimization
        </Link>
         <Link className={`${linkBase} ${isActive('/notifications') ? 'bg-surface text-white border border-white/10' : ''}`} href="/notifications">
          <span className="material-symbols-outlined">notifications</span>
          Notifications
        </Link>
        <Link className={`${linkBase} ${isActive('/profilesetting') ? 'bg-surface text-white border border-white/10' : ''}`} href="/profilesetting">
